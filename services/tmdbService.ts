  ...
  getMovieDetails: async (movieId: number): Promise<Movie | null> => {
    try {
      ...
      return {
        ...
        plot: movie.overview,
        director: credits.crew?.find((c: { job: string }) => c.job === 'Director')?.name || 'Unknown',
        cast: credits.cast?.slice(0, 5).map((c: { name: string }) => c.name) || [],
...
function inferMoodTags(genres: any[]): string[] {
  ...
  genres?.forEach((genre) => {
    ...
    moodMap[name]?.forEach((mood) => tags.add(mood));
  });
